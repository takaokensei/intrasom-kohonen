...
E-mail: caua.vitor.016@ufrn.edu.br.}%
...
(i)~deriva\c{c}\~ao anal\'itica do fator de infla\c{c}\~ao 2-D e generaliza\c{c}\~ao te\'orica 3-D;
(ii)~estudo multidataset com $N=30$ sementes e tamanhos de efeito;
(iii)~protocolo de conformidade execut\'avel (\texttt{src/conformance\_check.py}) para bibliotecas SOM; e
...
\begin{enumerate}
    \item \textbf{Deriva\c{c}\~ao Anal\'itica 2-D e 3-D}: Demonstra\c{c}\~ao do fator
    de infla\c{c}\~ao te\'orico $(1+\sqrt{2})/2 \approx 1{,}207$ (2-D) e generaliza\c{c}\~ao
    para U-arrays 3-D ($(6+12\sqrt{2}+8\sqrt{3})/26 \approx 1{,}416$).
    ...
    \item \textbf{Framework de Conformidade Execut\'avel}: Proposta e disponibiliza\c{c}\~ao
    de protocolo de testes de conformidade (\texttt{src/conformance\_check.py}).
...
tridimensionais (U-array), estabelecendo explicitamente o fator $1/\sqrt{2}$ para
...
\subsection{Deriva\c{c}\~ao do Limite Te\'orico de Infla\c{c}\~ao 2-D}
\label{subsec:fator}
...
assint\'otico superior} sob isotropia perfeita em 2-D.

\subsection{Extens\~ao Te\'orica a Sa\'idas 3-D: U-Array}
\label{subsec:fator_3d}
Costa \& Netto (2007) \cite{costa2007segmentacao} estenderam a U-matrix para
grades tridimensionais (\textit{U-array}), onde um neur\^onio interno possui
26 vizinhos no cubo $3 \times 3 \times 3$: 6 ortogonais ($s=1$), 12 diagonais
de face ($s=\sqrt{2}$) e 8 diagonais de canto/corpo ($s=\sqrt{3}$).

Sob um gradiente de pesos 3-D perfeitamente isotr\'opico, o U-array sem normaliza\c{c}\~ao
espacial resulta em:
\begin{equation}
    U_{\text{unnorm}}^{\text{3D}} = \frac{6(1) + 12(\sqrt{2}) + 8(\sqrt{3})}{26} d_{\perp} \approx 1{,}4164 \, d_{\perp}.
\end{equation}
Enquanto a formula\c{c}\~ao cl\'assica normalizada por $s_k$ produz $U_{\text{class}}^{\text{3D}} = d_{\perp}$.
Dessa forma, a omiss\~ao da normaliza\c{c}\~ao diagonal em um U-array 3-D gera uma
\textbf{infla\c{c}\~ao assint\'otica te\'orica de $41{,}64\%$} (raz\~ao $1{,}4164$).

Inspe\c{c}\~ao do c\'odigo-fonte da IntraSOM (v1.1.1) confirma que a biblioteca suporta
estritamente grades bidimensionais (\texttt{self.lattice} exige \texttt{"rect"} ou
\texttt{"hexa"}). Assim, a estrutura 3-D permanece como uma extens\~ao te\'orica
preditiva deste artigo, cuja valida\c{c}\~ao emp\'irica \'e declarada como Trabalho Futuro.
...
O estudo utiliza tr\^es datasets de refer\^encia selecionados para cobrir n\'iveis
crescentes de sobreposi\c{c}\~ao entre classes (baixa, m\'edia e alta), permitindo
avaliar a modera\c{c}\~ao da sobreposi\c{c}\~ao sobre a detectabilidade do efeito:
...
\begin{itemize}
    ...
    Dataset de baixa sobreposi\c{c}\~ao e separa\c{c}\~ao morfol\'ogica clara.
...
p\'ublico do projeto \cite{sandve2013ten, stodden2014implementing}.
...
confirmando vari\^ancia real nos codebooks ($\max |\mathbf{W}_{42} - \mathbf{W}_{43}| > 2{,}45$)
...
\begin{figure}[htbp]
    ...
    \caption{Compara\c{c}\~ao das U-matrizes $U_A$ (IntraSOM v1.1.1) e $U_B$
    (Costa \& Netto 2007) para SOM $10\times10$ RECT toroidal:
    (a) $U_A$ sem normaliza\c{c}\~ao; (b) $U_B$ com $1/\sqrt{2}$; (c) $|U_A - U_B|$.}
...
M\'edias com IC Bootstrap $95\%$. $p_\text{fdr}$: Benjamini-Hochberg ($q=0{,}05$). $r_\text{effect}$: Rosenthal ($|Z|/\sqrt{N}$). $^*p_\text{fdr}<0{,}05$.}
...
conforme o mapa cresce. A correla\c{c}\~ao de Pearson \'e elevada em todas as
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\linewidth]{fig2_systematic_divergence.pdf}
    \caption{Sistematicidade da diverg\^encia vs.\ dimens\~ao $N$:
    (a) Diverg\^encia relativa (\%); (b) Raz\~ao de escala $\overline{U}_A/\overline{U}_B$ e teto te\'orico $1{,}207$.}
...
rejei\c{c}\~oes $H_0$ ($p_\text{fdr}<0{,}05$, $N=30$), escala m\'edia e tamanho de efeito m\'aximo.}
\label{tab:generalizacao}
\setlength{\tabcolsep}{8pt}
\footnotesize
\begin{tabular}{lcccc}
\toprule
\textbf{Dataset} & \textbf{Pearson $r$} & \textbf{Rej. $H_0$} & \textbf{Escala M\'edia} & \textbf{$r_\text{eff}$ (max)} \\
\midrule
Synthetic Control & $0.998$ & 9/12 ($75\%$) & $1.193$ & $0.862$ \\
Wine              & $0.997$ & 8/12 ($67\%$) & $1.196$ & $1.000^\dagger$ \\
Digits            & $0.988$ & 3/12 ($25\%$) & $1.193$ & $0.679$ \\
\bottomrule
\multicolumn{5}{l}{\scriptsize $\dagger$ No modelo Wine SOM $7\times7$ toroidal, a vari\^ancia pareada zero ($\text{std}=0$) faz $d_z=0{,}000$ enquanto $r_\text{eff}=1{,}000$ (ver Se\c{c}\~ao~\ref{subsec:disc_downstream}).}
\end{tabular}
\end{table*}

A Fig.~\ref{fig:segmentation_ari} ilustra a concord\^ancia de segmenta\c{c}\~ao (ARI entre $U_A$ e $U_B$)
em fun\c{c}\~ao da dimens\~ao da grade, evidenciando como grades menores ($5\times5$) s\~ao mais sens\'iveis
\`a varia\c{c}\~ao no relevo local da U-matrix.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\linewidth]{fig3_segmentation_ari.pdf}
    \caption{Adjusted Rand Index (ARI) entre segmenta\c{c}\~oes $U_A$ e $U_B$ por dimens\~ao de grade $N$.}
...
\subsection{Framework de Conformidade Execut\'avel para Bibliotecas SOM}
\label{subsec:framework}
Com base nos achados deste estudo, prop\~oe-se um framework de conformidade
execut\'avel em 4 passos, disponibilizado no reposit\'orio como \texttt{src/conformance\_check.py}:
...
\begin{enumerate}
    ...
    \item \textbf{Testar com mapa sint\'etico isotr\'opico}: Gerar um mapa com pesos
    regulares onde $d_\text{diag}/d_\text{ort} = \sqrt{2}$, e verificar se $\overline{U}/d_\text{ort} = 1{,}000$.
    \item \textbf{Reportar a variante adotada}: Documentar explicitamente
    se a normaliza\c{c}\~ao diagonal \'e aplicada, para garantir comparabilidade \cite{flexer2001use}.
    \item \textbf{Testar por reprodu\c{c}\~ao cruzada}: Comparar resultados
    com implementa\c{c}\~oes de refer\^encia (ex: MATLAB SOM Toolbox) em pelo menos um dataset p\'ublico.
\end{enumerate}

O script \texttt{src/conformance\_check.py} executa o passo 2 em tempo real,
identificando automaticamente se uma biblioteca apresenta o vi\'es de infla\c{c}\~ao de $20{,}7\%$.
...
confirmados parametricamente por $d_z$ de Cohen atingindo at\'e $2{,}01$)
...
em fronteiras de decis\~ao. Ressalta-se que em 19 das 20 configura\c{c}\~oes
significativas observa-se estrita concord\^ancia entre $|d_z|$ e $r_\text{effect}$; no
\'unico caso at\'ipico (\textit{Wine}, SOM $7\times7$ toroidal), a diferen\c{c}a pareada
possui vari\^ancia zero entre sementes ($\text{std}=0$), fazendo $d_z$ colapsar
por defini\c{c}\~ao matem\'atica ($d_z = 0{,}000$) enquanto $r_\text{effect}$ atinge o
valor m\'aximo ($r_\text{effect} = 1{,}000$, $p_\text{fdr} = 5{,}18 \times 10^{-7}$)
devido \`a consist\^encia absoluta do efeito.
...
\begin{enumerate}
    \item \textbf{Escopo Geom\'etrico}: A an\'alise emp\'irica \'e restrita \`a malha
    retangular (RECT) 2-D. Malhas hexagonais (HEX) disp\~oem de simetria radial
    equi\^angula de 6 vizinhos e n\~ao apresentam essa distor\c{c}\~ao.
    \item \textbf{Suporte a Sa\'idas 3-D}: A extens\~ao a U-arrays 3-D derivada na
    Se\c{c}\~ao~\ref{subsec:fator_3d} permanece te\'orica, uma vez que a IntraSOM 1.1.1
    suporta apenas grades 2-D.
...
\begin{itemize}
    ...
    \texttt{build\_umatrix(normalize\_diagonals)}.
    \item \textbf{Valida\c{c}\~ao Emp\'irica 3-D}: Avaliar o fator de infla\c{c}\~ao
    te\'orico de $41{,}64\%$ em U-arrays 3-D utilizando uma implementa\c{c}\~ao de refer\^encia
    tridimensional.
    \item \textbf{Extens\~ao a Mais Bibliotecas}: Aplicar o framework de
    conformidade execut\'avel (\texttt{src/conformance\_check.py}) a outras bibliotecas SOM (MiniSom, SOMpy, MATLAB SOM Toolbox).
\end{itemize}
...
te\'orico $(1+\sqrt{2})/2 \approx 1{,}207$. A generaliza\c{c}\~ao te\'orica 3-D
estabeleceu o limite assint\'otico de $41{,}64\%$ para U-arrays tridimensionais.
O estudo de larga escala---1\,080 modelos SOM, $N=30$ sementes, 3 datasets---confirma a generalidade do achado.
...
A contribui\c{c}\~ao principal \'e quadrupla: (i)~a descoberta e quantifica\c{c}\~ao
rigorosa da diverg\^encia na IntraSOM v1.1.1; (ii)~a deriva\c{c}\~ao anal\'itica 2-D e 3-D dos limites te\'oricos de infla\c{c}\~ao;
(iii)~a proposi\c{c}\~ao de um framework de conformidade execut\'avel (\texttt{src/conformance\_check.py}); e (iv)~a proposta concreta de corre\c{c}\~ao da API.
